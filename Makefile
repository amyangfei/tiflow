...
LINUX := "Linux"
MAC   := "Darwin"
CDC_PKG := github.com/amyangfei/tiflow
DM_PKG := github.com/amyangfei/tiflow/dm
PACKAGE_LIST := go list ./... | grep -vE 'vendor|proto|ticdc\/tests|integration|testing_utils|pb|pbmock'
PACKAGE_LIST_WITHOUT_DM := $(PACKAGE_LIST) | grep -vE 'github.com/amyangfei/tiflow/dm'
DM_PACKAGE_LIST := go list github.com/amyangfei/tiflow/dm/... | grep -vE 'pb|pbmock|dm/cmd'
...
integration_test_build: check_failpoint_ctl
	...
	$(GOTEST) -ldflags '$(LDFLAGS)' -c -cover -covermode=atomic \
		-coverpkg=github.com/amyangfei/tiflow/... \
		-o bin/cdc.test github.com/amyangfei/tiflow/cmd/cdc \
	|| { $(FAILPOINT_DISABLE); exit 1; }
	$(GOBUILD) -ldflags '$(LDFLAGS)' -o bin/cdc ./cmd/cdc/main.go \
	|| { $(FAILPOINT_DISABLE); exit 1; }
	$(FAILPOINT_DISABLE)
...
dm_integration_test_build: check_failpoint_ctl
	$(FAILPOINT_ENABLE)
	$(GOTEST) -ldflags '$(LDFLAGS)' -c -cover -covermode=atomic \
		-coverpkg=github.com/amyangfei/tiflow/dm/... \
		-o bin/dm-worker.test github.com/amyangfei/tiflow/dm/cmd/dm-worker \
		|| { $(FAILPOINT_DISABLE); exit 1; }
	$(GOTEST) -ldflags '$(LDFLAGS)' -c -cover -covermode=atomic \
		-coverpkg=github.com/amyangfei/tiflow/dm/... \
		-o bin/dm-master.test github.com/amyangfei/tiflow/dm/cmd/dm-master \
		|| { $(FAILPOINT_DISABLE); exit 1; }
	$(GOTESTNORACE) -ldflags '$(LDFLAGS)' -c -cover -covermode=count \
		-coverpkg=github.com/amyangfei/tiflow/dm/... \
		-o bin/dmctl.test github.com/amyangfei/tiflow/dm/cmd/dm-ctl \
		|| { $(FAILPOINT_DISABLE); exit 1; }
	$(GOTEST) -ldflags '$(LDFLAGS)' -c -cover -covermode=atomic \
		-coverpkg=github.com/amyangfei/tiflow/dm/... \
		-o bin/dm-syncer.test github.com/amyangfei/tiflow/dm/cmd/dm-syncer \
		|| { $(FAILPOINT_DISABLE); exit 1; }
	$(FAILPOINT_DISABLE)
