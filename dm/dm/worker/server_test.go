...
func (t *testServer) TestHandleSourceBoundAfterError(c *C) {
	...
	// enable failpoint
	c.Assert(failpoint.Enable("github.com/amyangfei/tiflow/dm/pkg/ha/FailToGetSourceCfg", `return(true)`), IsNil)
	sourceCfg := loadSourceConfigWithoutPassword(c)
	...
	cancel()
	wg.Wait()
	c.Assert(failpoint.Disable("github.com/amyangfei/tiflow/dm/pkg/ha/FailToGetSourceCfg"), IsNil)
