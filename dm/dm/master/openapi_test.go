...
import (
	...
	"github.com/amyangfei/tiflow/dm/openapi/fixtures"
...
func (t *openAPISuite) TestTaskAPI(c *check.C) {
	ctx, cancel := context.WithCancel(context.Background())
	s := setupServer(ctx, c)
	c.Assert(failpoint.Enable("github.com/amyangfei/tiflow/dm/dm/master/MockSkipAdjustTargetDB", `return(true)`), check.IsNil)
	...
	c.Assert(failpoint.Disable("github.com/amyangfei/tiflow/dm/dm/master/MockSkipAdjustTargetDB"), check.IsNil)
