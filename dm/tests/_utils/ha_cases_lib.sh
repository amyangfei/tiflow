...
function isolate_master() {
	...
	if [ $2 = "isolate" ]; then
		export GO_FAILPOINTS="github.com/amyangfei/tiflow/dm/dm/master/FailToElect=return(\"master$1\")"
	fi
...
function isolate_worker() {
	...
	if [ $2 = "isolate" ]; then
		export GO_FAILPOINTS="github.com/amyangfei/tiflow/dm/dm/worker/FailToKeepAlive=return(\"worker$1\")"
	fi
