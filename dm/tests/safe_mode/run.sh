...
function safe_mode_recover() {
	...
	while [ $i -lt 5 ]; do
		...
		export GO_FAILPOINTS="github.com/amyangfei/tiflow/dm/syncer/SafeModeExit=return($i)"
		run_dm_worker $WORK_DIR/worker1 $WORKER1_PORT $cur/conf/dm-worker1.toml
		check_rpc_alive $cur/../bin/check_worker_online 127.0.0.1:$WORKER1_PORT
		run_dm_worker $WORK_DIR/worker2 $WORKER2_PORT $cur/conf/dm-worker2.toml
		check_rpc_alive $cur/../bin/check_worker_online 127.0.0.1:$WORKER2_PORT
...
function run() {
	...
	export GO_FAILPOINTS='github.com/amyangfei/tiflow/dm/syncer/ReSyncExit=return(true)'
	run_dm_master $WORK_DIR/master $MASTER_PORT $cur/conf/dm-master.toml
	check_rpc_alive $cur/../bin/check_master_online 127.0.0.1:$MASTER_PORT
	run_dm_worker $WORK_DIR/worker1 $WORKER1_PORT $cur/conf/dm-worker1.toml
	check_rpc_alive $cur/../bin/check_worker_online 127.0.0.1:$WORKER1_PORT
	...
	export GO_FAILPOINTS='github.com/amyangfei/tiflow/dm/syncer/ShardSyncedExecutionExit=return(true);github.com/amyangfei/tiflow/dm/syncer/SafeModeInitPhaseSeconds=return(300)'
	...
	while [ $i -lt 10 ]; do
		...
		if [ "$(check_port_return $WORKER1_PORT)" == "0" ]; then
			...
			export GO_FAILPOINTS='github.com/amyangfei/tiflow/dm/syncer/SafeModeInitPhaseSeconds=return(0)'
			run_dm_worker $WORK_DIR/worker1 $WORKER1_PORT $cur/conf/dm-worker1.toml
			check_rpc_alive $cur/../bin/check_worker_online 127.0.0.1:$WORKER1_PORT
			check_instance_id="1"
