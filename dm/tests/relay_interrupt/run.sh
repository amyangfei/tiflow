...
function run() {
	failpoints=(
		...
		"github.com/amyangfei/tiflow/dm/pkg/utils/GetGlobalVariableFailed=return(\"server_uuid,1152\")"
		"github.com/amyangfei/tiflow/dm/pkg/utils/GetSessionVariableFailed=return(\"sql_mode,1152\")"
	)
	...
	for ((i = 0; i < ${#failpoints[@]}; i++)); do
		...
		kill_dm_worker
		export GO_FAILPOINTS="github.com/amyangfei/tiflow/dm/relay/GetEventFromLocalFailed=return()"
		run_dm_worker $WORK_DIR/worker1 $WORKER1_PORT $cur/conf/dm-worker1.toml
		check_rpc_alive $cur/../bin/check_worker_online 127.0.0.1:$WORKER1_PORT
		prepare_data2 $i
		sleep 8
