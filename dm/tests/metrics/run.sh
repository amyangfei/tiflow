...
function run() {
	...
	inject_points=(
		"github.com/amyangfei/tiflow/dm/syncer/BlockDDLJob=return(1)"
		"github.com/amyangfei/tiflow/dm/syncer/ShowLagInLog=return(1)" # test lag metric >= 1 beacuse we inject BlockDDLJob(ddl) to sleep(1)
	)
	...
	inject_points=(
		"github.com/amyangfei/tiflow/dm/syncer/BlockExecuteSQLs=return(2)"
		"github.com/amyangfei/tiflow/dm/syncer/ShowLagInLog=return(2)" # test lag metric >= 2 beacuse we inject BlockExecuteSQLs to sleep(2) although skip lag is 0 (locally), but we use that lag of all dml/skip lag, so lag still >= 2
	)
	export GO_FAILPOINTS="$(join_string \; ${inject_points[@]})"
	...
	inject_points=(
		"github.com/amyangfei/tiflow/dm/syncer/noJobInQueueLog=return()"
		"github.com/amyangfei/tiflow/dm/syncer/IgnoreSomeTypeEvent=return(\"HeartbeatEvent\")"
	)
	export GO_FAILPOINTS="$(join_string \; ${inject_points[@]})"
	rm -rf $WORK_DIR/worker1/log/dm-worker.log # clean up the old log
	run_dm_worker $WORK_DIR/worker1 $WORKER1_PORT $cur/conf/dm-worker1.toml
