...
function run() {
	...
	export GO_FAILPOINTS="github.com/amyangfei/tiflow/dm/syncer/GetEventErrorInTxn=13*return(3)"
	...
	# with a 5 rows insert txn: 1 * FormatDesc + 1 * PreviousGTID + 1 * GTID + 1 * BEGIN + 5 * (Table_map + Write_rows) + 1 * XID
	# here we fail at the third write rows event, sync should retry and auto recover without any duplicate event
	export GO_FAILPOINTS="github.com/amyangfei/tiflow/dm/relay/RelayGetEventFailed=15*return(3);github.com/amyangfei/tiflow/dm/relay/RelayAllowRetry=return"
