...
function test_save_checkpoint_failed() {
	...
	export GO_FAILPOINTS="github.com/amyangfei/tiflow/dm/loader/loaderCPUpdateOffsetError=return()"
...
function run() {
	...
	run_sql_file $WORK_DIR/db1.prepare.sql $MYSQL_HOST1 $MYSQL_PORT1 $MYSQL_PASSWORD1
	THRESHOLD=1024
	export GO_FAILPOINTS="github.com/amyangfei/tiflow/dm/loader/LoadExceedOffsetExit=return($THRESHOLD)"
	...
	export GO_FAILPOINTS='github.com/amyangfei/tiflow/dm/loader/LoadExecCreateTableFailed=3*return("1213")' # ER_LOCK_DEADLOCK, retryable error code
	run_dm_worker $WORK_DIR/worker1 $WORKER1_PORT $cur/conf/dm-worker1.toml
	check_rpc_alive $cur/../bin/check_worker_online 127.0.0.1:$WORKER1_PORT
