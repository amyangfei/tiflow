...
func testSorter(ctx context.Context, c *check.C, sorter sorter.EventSorter, count int) error {
	err := failpoint.Enable("github.com/amyangfei/tiflow/cdc/sorter/unified/sorterDebug", "return(true)")
	if err != nil {
		log.Panic("Could not enable failpoint", zap.Error(err))
	}

	c.Assert(failpoint.Enable("github.com/amyangfei/tiflow/pkg/util/InjectCheckDataDirSatisfied", ""), check.IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/amyangfei/tiflow/pkg/util/InjectCheckDataDirSatisfied"), check.IsNil)
	}()
...
func (s *sorterSuite) TestSorterCancelRestart(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/sorter/unified/asyncFlushStartDelay", "sleep(100)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/sorter/unified/asyncFlushStartDelay")
	}()

	// enable the failpoint to simulate delays
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/sorter/unified/asyncFlushInProcessDelay", "1%sleep(1)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/sorter/unified/asyncFlushInProcessDelay")
	}()
...
func (s *sorterSuite) TestSorterIOError(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/sorter/unified/InjectErrorBackEndAlloc", "return(true)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/sorter/unified/InjectErrorBackEndAlloc")
	}()
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/sorter/unified/InjectErrorBackEndWrite", "return(true)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/sorter/unified/InjectErrorBackEndWrite")
	}()
...
func (s *sorterSuite) TestSorterErrorReportCorrect(c *check.C) {
	...
	// enable the failpoint to simulate backEnd allocation error (usually would happen when creating a file)
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/sorter/unified/InjectHeapSorterExitDelay", "sleep(2000)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/sorter/unified/InjectHeapSorterExitDelay")
	}()
