...
func (s *clientSuite) testStreamRecvWithError(c *check.C, failpointStr string) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientStreamRecvError", failpointStr)
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientStreamRecvError")
	}()
	baseAllocatedID := currentRequestID()
	lockresolver := txnutil.NewLockerResolver(kvStorage)
	isPullInit := &mockPullerInit{}
...
func (s *clientSuite) TestStreamRecvWithErrorAndResolvedGoBack(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientStreamRecvError", "1*return(\"\")")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientStreamRecvError")
	}()
...
func (s *clientSuite) TestIncompatibleTiKV(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientDelayWhenIncompatible", "return(true)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientDelayWhenIncompatible")
	}()
	lockresolver := txnutil.NewLockerResolver(kvStorage)
	isPullInit := &mockPullerInit{}
	grpcPool := NewGrpcPoolImpl(ctx, &security.Credential{})
...
func (s *clientSuite) TestResolveLock(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientResolveLockInterval", "return(3)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientResolveLockInterval")
	}()
	baseAllocatedID := currentRequestID()
	lockresolver := txnutil.NewLockerResolver(kvStorage)
	isPullInit := &mockPullerInit{}
...
func (s *clientSuite) testEventCommitTsFallback(c *check.C, events []*cdcpb.ChangeDataEvent) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientErrUnreachable", "return(true)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientErrUnreachable")
	}()
	baseAllocatedID := currentRequestID()
	lockresolver := txnutil.NewLockerResolver(kvStorage)
	isPullInit := &mockPullerInit{}
...
func (s *clientSuite) testEventAfterFeedStop(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientSingleFeedProcessDelay", "1*sleep(2000)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientSingleFeedProcessDelay")
	}()
	baseAllocatedID := currentRequestID()
	lockresolver := txnutil.NewLockerResolver(kvStorage)
	isPullInit := &mockPullerInit{}
...
func (s *clientSuite) TestFailRegionReentrant(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientRegionReentrantError", "1*return(\"ok\")->1*return(\"error\")")
	c.Assert(err, check.IsNil)
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientRegionReentrantErrorDelay", "sleep(500)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientRegionReentrantError")
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientRegionReentrantErrorDelay")
	}()
	baseAllocatedID := currentRequestID()
...
func (s *clientSuite) TestClientV1UnlockRangeReentrant(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientStreamRecvError", "1*return(\"injected stream recv error\")")
	c.Assert(err, check.IsNil)
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientPendingRegionDelay", "1*sleep(0)->1*sleep(2000)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientStreamRecvError")
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientPendingRegionDelay")
	}()
	lockresolver := txnutil.NewLockerResolver(kvStorage)
	isPullInit := &mockPullerInit{}
	grpcPool := NewGrpcPoolImpl(ctx, &security.Credential{})
...
func (s *clientSuite) testClientErrNoPendingRegion(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientStreamRecvError", "1*return(\"injected error\")")
	c.Assert(err, check.IsNil)
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientPendingRegionDelay", "1*sleep(0)->2*sleep(1000)")
	c.Assert(err, check.IsNil)
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientStreamCloseDelay", "sleep(2000)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientStreamRecvError")
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientPendingRegionDelay")
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientStreamCloseDelay")
	}()
	lockresolver := txnutil.NewLockerResolver(kvStorage)
	isPullInit := &mockPullerInit{}
	grpcPool := NewGrpcPoolImpl(ctx, &security.Credential{})
...
func (s *clientSuite) testKVClientForceReconnect(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientResolveLockInterval", "return(1)")
	c.Assert(err, check.IsNil)
...
func (s *clientSuite) TestConcurrentProcessRangeRequest(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientMockRangeLock", "1*return(20)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientMockRangeLock")
	}()
	lockresolver := txnutil.NewLockerResolver(kvStorage)
	isPullInit := &mockPullerInit{}
	grpcPool := NewGrpcPoolImpl(ctx, &security.Credential{})
...
func (s *clientSuite) TestEvTimeUpdate(c *check.C) {
	...
	err = failpoint.Enable("github.com/amyangfei/tiflow/cdc/kv/kvClientCheckUnInitRegionInterval", "return(2)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/amyangfei/tiflow/cdc/kv/kvClientCheckUnInitRegionInterval")
		reconnectInterval = originalReconnectInterval
	}()
